...
var dayModule = (function() {
    ...
    Day.prototype.addAttraction = function(attraction) {
        ...
        // // Add attractions to db Day's relevant attractions array
        // methods.addAttract(this.number, attraction);
    };
    ...
    var methods = {
        // Creates a new day object, optionally based on a Day document
        create: function(databaseDay) {
            return new Day(databaseDay);
        },

        // POSTs a day object to the database (as a Day document) with AJAX
        add: function(dayObj) {
            $.post('../api/days', { number: dayObj.number });
        },

        delete: function(dayObj) {
            console.log("Deleting ", dayObj.number);
            console.log('../api/days/' + dayObj.number);
            // $.delete('../api/days/' + dayObj.number);
            $.ajax({
                url: '../api/days/' + dayObj.number,
                type: 'DELETE'
            });
        }
